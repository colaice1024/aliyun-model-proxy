...
async function proxyModelPoolRequest(
  input: ModelPoolProxyInput,
): Promise<Response> {
  ...
  for (const candidate of candidates) {
    ...
    const upstreamResponse = await fetch(
      buildUpstreamUrl(input.upstreamBaseUrl, input.pathWithSearch),
      {
    ...
    const exhaustionCheck = await readFreeTierExhaustion(
      upstreamResponse.clone(),
    );
    logUpstreamRetryDecision(
...
function buildUpstreamHeaders(
  ...
  const blockedHeaderNames = new Set(
    blockedRequestHeaders.map((header) => header.toLowerCase()),
  );
  ...
  if (
    config.upstreamAuthMode === "authorization" ||
    config.upstreamAuthMode === "both"
  ) {
    headers.set("authorization", `Bearer ${apiKey}`);
  }

  if (
    config.upstreamAuthMode === "x-api-key" ||
    config.upstreamAuthMode === "both"
  ) {
...
async function readFreeTierExhaustion(
  response: Response,
): Promise<FreeTierExhaustionCheck> {
...
function extractFreeTierExhaustion(
  payload: unknown,
  status?: number,
): FreeTierExhaustion | null {
  ...
  const quotaExceeded =
    status === 429 && /you exceeded your current quota/i.test(message);
  const throttlingQuotaExceeded =
    status === 429 &&
    (code === "Throttling" || type === "Throttling") &&
